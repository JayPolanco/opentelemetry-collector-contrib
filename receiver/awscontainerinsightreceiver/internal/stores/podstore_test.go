...
func TestPodStore_decorateMem(t *testing.T) {
	...
	assert.Equal(t, uint64(104857600), metric.GetField("pod_memory_request").(uint64))
	assert.Equal(t, uint64(104857600), metric.GetField("pod_memory_limit").(uint64))
	assert.Equal(t, float64(25), metric.GetField("pod_memory_reserved_capacity").(float64))
	assert.Equal(t, float64(10), metric.GetField("pod_memory_utilization_over_pod_limit").(float64))
